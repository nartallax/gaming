...
#define BLOWFISH_ROUNDS_COUNT 16
...
void Blowfish_Encrypt(BLOWFISH_CTX *ctx, uint32_t *xl, uint32_t *xr){
  ...
  for (i = 0; i < BLOWFISH_ROUNDS_COUNT; ++i) {
    Xl = Xl ^ ctx->P[i];
    Xr = F(ctx, Xl) ^ Xr;
  ...
  Xr = Xr ^ ctx->P[BLOWFISH_ROUNDS_COUNT];
  Xl = Xl ^ ctx->P[BLOWFISH_ROUNDS_COUNT + 1];
...
void Blowfish_Decrypt(BLOWFISH_CTX *ctx, uint32_t *xl, uint32_t *xr){
  ...
  for (i = BLOWFISH_ROUNDS_COUNT + 1; i > 1; --i) {
    Xl = Xl ^ ctx->P[i];
    Xr = F(ctx, Xl) ^ Xr;
...
void Blowfish_Init(BLOWFISH_CTX *ctx, const uint8_t *key, int32_t keyLen) {
  ...
  j = 0;
  for (i = 0; i < BLOWFISH_ROUNDS_COUNT + 2; ++i) {
    data = 0x00000000;
    for (k = 0; k < 4; ++k) {
      data = (data << 8) | key[j];
      j = j + 1;
  ...
  for (i = 0; i < BLOWFISH_ROUNDS_COUNT + 2; i += 2) {
